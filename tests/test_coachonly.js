...
ok(await m.evaluate(()=>document.querySelectorAll('.mode').length)===3,
  `ثلاثة أزرار لا أكثر — المحادثة والاستماع والقراءة (${await m.evaluate(()=>document.querySelectorAll('.mode').length)})`);
...
ok(t.includes('صفحة إلياس')&&t.includes('ابدأ المحادثة')&&t.includes('فهم الاستماع')&&t.includes('فهم المقروء'),'المحادثة والاستماع والقراءة');
ok(!t.includes('محاكاة شاملة')&&!t.includes('إملاء'),'وبقيّة الأقسام مخفيّة');
ok(await e.evaluate(()=>document.querySelectorAll('.mode').length)===3,'ثلاثة أزرار لا أكثر');
...
['برنامج موهبة المكثّف','الاستدلال اللغوي','محاكاة شاملة','وضع التقوية','متابعة التقدّم',
 ...
 'تكلّمي بالإنجليزية','فهم الاستماع','فهم المقروء','جمل من كتابك','استراتيجيات موهبة'].forEach(g=>ok(t.includes(g),`«${g}» باقٍ عندها`));
