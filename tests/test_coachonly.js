...
ok(await m.evaluate(()=>document.querySelectorAll('.mode').length)===4,
  `أربعة أزرار لا أكثر — المحادثة والاستماع والقراءة ولعبة الأزواج (${await m.evaluate(()=>document.querySelectorAll('.mode').length)})`);
...
ok(t.includes('صفحة إلياس')&&t.includes('ابدأ المحادثة')&&t.includes('فهم الاستماع')&&t.includes('فهم المقروء')&&t.includes('لعبة الاستماع'),'المحادثة والاستماع والقراءة ولعبة الأزواج');
