...
ok(await m.evaluate(()=>document.querySelectorAll('.mode').length)===8,
  `ثمانية أزرار لا أكثر — المحادثة والاستماع والقراءة والكتابة ودقّة القواعد ونمط STEP ولعبة الأزواج والفيديو (${await m.evaluate(()=>document.querySelectorAll('.mode').length)})`);
...
ok(await e.evaluate(()=>document.querySelectorAll('.mode').length)===8,'ثمانية أزرار لا أكثر');
