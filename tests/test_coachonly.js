...
ok(t.includes('إحصاء ١٠٢'),'وزرّ إحصاء ١٠٢ — لمحمد وحده لا للثلاثة: مقرّرٌ جامعي بعينه لا مهارةٌ عامّة');
...
// أحد عشر زرّاً مشتركاً: الثمانية السابقة + تركيز الأسبوع (تسلسل التدريس) + لوحة القياس
// + المحاكاة — ٢١ أغسطس. والعدد يبقى مكتوباً عمداً رغم كونه «فخّاً صامتاً» (درس ١٨ أغسطس):
// هو الحارس الوحيد الذي يمنع تسرّب قسمٍ من أقسام هيا إلى صفحة الأخوين، فيُحدَّث مع كل زرٍّ مقصود.
// وصفحة محمد صارت تزيد عليها واحداً (إحصاء ١٠٢، ٢٥ أغسطس) — أوّل افتراقٍ بين الصفحتين،
// فالعدد هنا ثلاثة عشر وعند إلياس اثنا عشر، والفرق نفسه محروسٌ لا مصادفة.
// ٢٦ أغسطس: أُضيف «بناء الجملة» للأخوين (كان محبوساً في خطّة هيا وليس من منهج السادس)،
// فزاد كلٌّ واحداً — والعدد يُحدَّث مع كل زرٍّ مقصود، فهو حارسُ التسرّب لا رقمٌ عابر.
ok(await m.evaluate(()=>document.querySelectorAll('.mode').length)===13,
  `ثلاثة عشر زرّاً لا أكثر — الاثنا عشر المشتركة + إحصاء ١٠٢ الخاصّ بمحمد (${await m.evaluate(()=>document.querySelectorAll('.mode').length)})`);
...
ok(!t.includes('إحصاء ١٠٢'),'ولا يرى إحصاء ١٠٢ — ليس مقرَّره، والطلب كان «في صفحة محمد» نصّاً');
ok(await e.evaluate(()=>document.querySelectorAll('.mode').length)===12,'اثنا عشر زرّاً لا أكثر — بلا زرّ محمد');
ok(t.includes('بناء الجملة'),'ويرى بناء الجملة — أُخرج من خطّة هيا لأنه ليس من منهج السادس');
...
ok(!t.includes('إحصاء ١٠٢'),'ولا ترى إحصاء ١٠٢ — مقرّرٌ جامعي، وهي في السادس الابتدائي');
