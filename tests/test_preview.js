...
console.log('\n٥) والإجراء نفسه: الجذر من main دائماً، والتجريب إضافةٌ لا شرط');
{
  ...
  // ===== ووسمُ النسخة يُختم آلياً — العيب وقع حيّاً مرّةً ثالثة =====
  // بطاقة جهاز هيا (٣٠ أغسطس) وصلت تقول «بناء:build-20260825-0100» بينما تحمل
  // الصفحةُ شيفرة اليوم — لأن الوسم ثابتٌ يُكتب بيد. فتعذّر الحكم من الوسم أصلاً:
  // أنسخةٌ قديمة أم وسمٌ قديم فوق نسخةٍ جديدة؟ فنُزع التذكّر من مساره.
  ok(/Stamp build id from commit/.test(y),'الوسم يُختم في الإجراء لا بيدٍ');
  ok(/rev-parse --short HEAD/.test(y),'من بصمة الالتزام — فيتحرّك مع كل شحنة');
  ok(y.indexOf('Stamp build id')<y.indexOf('Strip nested git metadata'),
     'وقبل نزع git — وإلّا تعذّرت قراءة بصمة الفرع');
  ok(/pv-\$\(git -C preview rev-parse/.test(y),
     'ووسمُ التجريب مسبوقٌ بـpv- فلا يُقرأ على أنه الإنتاج');
  ok(/لا وسم BUILD في/.test(y),'ويسقط التشغيل إن اختفى الوسم — لا ختمٌ صامتٌ لا يقع');
  ok(/var BUILD="/.test(fs.readFileSync(path.join(ROOT,'index.html'),'utf8')),
     'والنمط الذي يبحث عنه موجودٌ فعلاً في index.html');
