...
await page.evaluate(()=>startAudioDiag());
t=await page.textContent('#app');
// ===== النصّ تغيّر عمداً، والمعنى هو ما يُثبَّت — ٢٩ أغسطس =====
// كانت العبارة «لا يوجد صوتٌ سحابي/طبيعي متاح على هذا الجهاز»، فنقل صاحب المشروع
// أن التطبيق «يقول لا يوجد صوت طبيعي» — أي أنها تُقرأ عطلاً وهي وصفٌ لحالةٍ عادية.
// فأُبدلت بنصٍّ يقول الحقيقة نفسها بلا إفزاع. والاختبار كان يُثبّت **العبارة** فكسره
// تغييرٌ مشروع؛ فصار يُثبّت **ما يجب أن يبقى صحيحاً**: لا يُدَّعى سحابيٌّ غير موجود،
// ويُقال صراحةً إن الأصوات كلّها محلّية.
ok(!t.includes('☁️ سحابي'),'الصراحة على التجميل — لا شارةَ «سحابي» وما من سحابيّ');
ok(/كل الأصوات هنا محلّية/.test(t),'ويُقال صراحةً إنها محلّية كلُّها');
ok(/Samantha/.test(t),'ومع ذلك يُصرَّح بالصوت المختار — فوجودُه ليس عطلاً');
await page.close();
