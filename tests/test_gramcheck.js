...
const mLv=await m.evaluate(()=>({level:profileOf().level,decl:(PROFILES.filter(p=>p.id==='mohammed')[0]||{}).level,btn:document.body.innerText.indexOf('دقّة القواعد')>=0}));
...
const mBank=await m.evaluate(()=>{const L=profileOf().level;
  return gramItems.every(x=>x.lv===L||x.lv===L+'+')});
ok(mBank,`وجلسته من بنك ${mLv.level} (ومعه تمديد ${mLv.level}+) وحده`);
const e=await mk('elias.html');
const eLv=await e.evaluate(()=>({level:profileOf().level,decl:(PROFILES.filter(p=>p.id==='elias')[0]||{}).level,btn:document.body.innerText.indexOf('دقّة القواعد')>=0}));
