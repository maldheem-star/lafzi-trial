...
const mLv=await m.evaluate(()=>({level:profileOf().level,decl:(PROFILES.filter(p=>p.id==='mohammed')[0]||{}).level,btn:document.body.innerText.indexOf('فهم الاستماع')>=0}));
...
const mBank=await m.evaluate(()=>listenItems.every(x=>x.lv===profileOf().level));
ok(mBank,`وجلسته من بنك ${mLv.level} وحده`);

const e=await mk('elias.html');
const eLv=await e.evaluate(()=>({level:profileOf().level,decl:(PROFILES.filter(p=>p.id==='elias')[0]||{}).level,btn:document.body.innerText.indexOf('فهم الاستماع')>=0}));
