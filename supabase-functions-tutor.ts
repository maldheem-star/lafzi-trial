...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (let ci = 0; ci < candidates.length; ci++) {
      ...
      // **وحدُّ ما يُصلحه هذا القيد يُقال بالقياس لا يُعمَّم**: السقف هنا يُشتقّ من
      // اسم النموذج، فالقيد **لا يبلغ إلا gpt-oss** (١٢٠٠ ⇒ ٩٥٠). وQwen — وهو
      // المضبوط في السرّ بحسب ما وُثِّق (١٨ أغسطس) — سقفُه ٥٠٠/٧٠٠ **تحت الحدّ
      // أصلاً**، فلا يتغيّر له شيء. أي أن `tutor_quota` الذي رآه إلياس **الأرجح أنه
      // `rate_limited` العارض لا `too_large` الدائم**، وهذا القيد وقايةٌ لمسار
      // الاحتياط لا علاجٌ لما وقع له. **والذي يحسم أيَّهما هو `quotaSummary` أدناه**،
      // ولهذا هو الأهمّ في هذا التعديل لا السقف.
